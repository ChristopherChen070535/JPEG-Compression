...
// readRLEBlock 總共有幾個8X8
RLEBlock *readRLEBlock(const char *line)
{
    int skip, value;
    int capacity = 10;
    ...
    while (sscanf(ptr, "%d %d%n", &skip, &value, &read) == 2)
    {
        if (size + 2 > capacity)
        {
            capacity *= 2;
        ...
        rle_data[size++] = skip;
        rle_data[size++] = value;
        ptr += read; // Move the pointer forward
        if (skip == 0 && value == 0)
            break; // End of block
    }

    RLEBlock *block = (RLEBlock *)malloc(sizeof(RLEBlock));
    block->size = size;
    block->data = rle_data;
    return block;
}
...
// 讀取RLE檔案
RLEImage readRLEFile(const char *filename, int *height, int *width)
{
    FILE *file = fopen(filename, "r");
    if (file == NULL)
    {
    ...
    int DCT_M = (*height + 7) / 8;
    int DCT_N = (*width + 7) / 8;
    char line[1024];

    RLEImage img;
    // Allocate memory for RLEBlocks
    img.Y = (RLEBlock **)malloc(DCT_M * DCT_N * sizeof(RLEBlock *));
    img.Cb = (RLEBlock **)malloc(DCT_M * DCT_N * sizeof(RLEBlock *));
    img.Cr = (RLEBlock **)malloc(DCT_M * DCT_N * sizeof(RLEBlock *));

    // Reading RLE data for each block
    for (int m = 0; m < DCT_M; m++)
    {
        for (int n = 0; n < DCT_N; n++)
        {
            // Read RLE data for Y component
            fgets(line, sizeof(line), file);
            img.Y[m * DCT_N + n] = readRLEBlock(line);

            // Read RLE data for Cb component
            fgets(line, sizeof(line), file);
            img.Cb[m * DCT_N + n] = readRLEBlock(line);

            // Read RLE data for Cr component
            fgets(line, sizeof(line), file);
            img.Cr[m * DCT_N + n] = readRLEBlock(line);
        }
    }
...
void freeRLEImage(RLEImage img, int height, int width)
{
    int DCT_M = (height + 7) / 8;
    int DCT_N = (width + 7) / 8;
    ...
    for (int m = 0; m < DCT_M; m++)
    {
        for (int n = 0; n < DCT_N; n++)
        {
            free(img.Y[m * DCT_N + n]->data);
            free(img.Y[m * DCT_N + n]);
            free(img.Cb[m * DCT_N + n]->data);
            free(img.Cb[m * DCT_N + n]);
            free(img.Cr[m * DCT_N + n]->data);
            free(img.Cr[m * DCT_N + n]);
        }
    }
...
// 分配記憶體給 UnRLEData
UnRLEData allocateUnRLEData(int H, int W, int M, int N)
{
    UnRLEData data;
    data.Y = (double ***)malloc(M * sizeof(double **));
    data.Cb = (double ***)malloc(M * sizeof(double **));
    data.Cr = (double ***)malloc(M * sizeof(double **));

    for (int m = 0; m < M; m++)
    {
        data.Y[m] = (double **)malloc(N * sizeof(double *));
        data.Cb[m] = (double **)malloc(N * sizeof(double *));
        data.Cr[m] = (double **)malloc(N * sizeof(double *));
        for (int n = 0; n < N; n++)
        {
            data.Y[m][n] = (double *)calloc(H * W, sizeof(double));
            data.Cb[m][n] = (double *)calloc(H * W, sizeof(double));
            data.Cr[m][n] = (double *)calloc(H * W, sizeof(double));
        }
    }
    return data;
}
...
// 釋放記憶體
void freeUnRLEData(UnRLEData data, int M, int N)
{
    for (int m = 0; m < M; m++)
    {
        for (int n = 0; n < N; n++)
...
// unRLE 函數
void unRLE(RLEImage *rleImage, UnRLEData *unrleData, int M, int N)
{
    for (int m = 0; m < M; m++)
    {
        for (int n = 0; n < N; n++)
        {
            ...
            while (i < rleImage->Y[index]->size)
            {
                ofs += rleImage->Y[index]->data[i++];
                unrleData->Y[m][n][ofs] = rleImage->Y[index]->data[i++];
            }
            i = 0, ofs = 0;
            while (i < rleImage->Cb[index]->size)
            {
                ofs += rleImage->Cb[index]->data[i++];
                unrleData->Cb[m][n][ofs] = rleImage->Cb[index]->data[i++];
            }
            i = 0, ofs = 0;
            while (i < rleImage->Cr[index]->size)
            {
                ofs += rleImage->Cr[index]->data[i++];
                unrleData->Cr[m][n][ofs] = rleImage->Cr[index]->data[i++];
            }
        }
    }
}
// un-zigzag 函數
void unZigzag(double ***component, int M, int N)
{
    double temp[DCT_H][DCT_W];
    ...
    for (int m = 0; m < M; m++)
    {
        for (int n = 0; n < N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                ...
                {
                    // 乘上反矩陣
                    temp[i][j] = component[m][n][zz_matrix[i * DCT_W + j] - 1];
                }
            }
            ...
            for (int i = 0; i < DCT_H; i++)
            {
                for (int j = 0; j < DCT_W; j++)
                {
                    component[m][n][i * DCT_W + j] = temp[i][j];
                }
            }
        }
    }
...
// un-DPCM函式
void unDPCM(double ***component, int M, int N)
{
    // vertical blocks
    for (int m = 1; m < M; m++)
    {
...
// Function to allocate memory for the final image
double **allocateImageMemory(int height, int width)
{
...
// Function to free memory allocated for the final image
void freeImageMemory(double **image, int height)
{
    for (int i = 0; i < height; i++)
    {
        free(image[i]);
...
int main(int argc, char *argv[])
{
    ...
    else if (command == 2)
    {
        ...
        int height, width;
        RLEImage rleImage = readRLEFile(rlefile, &height, &width);

        // 調整後的尺寸和塊數量
        int addheight = (height % 8 > 0) ? height + 8 - (height % 8) : height;
        int addwidth = (width % 8 > 0) ? width + 8 - (width % 8) : width;
        int M = addheight / 8;
        int N = addwidth / 8;

        UnRLEData unrleData = allocateUnRLEData(8, 8, M, N);
        // 調用 unRLE 函數
        unRLE(&rleImage, &unrleData, M, N);

        // 執行 unZigzag
        unZigzag(unrleData.Y, M, N);
        unZigzag(unrleData.Cb, M, N);
        unZigzag(unrleData.Cr, M, N);

        // 執行 un-DPCM
        unDPCM(unrleData.Y, M, N);
        unDPCM(unrleData.Cb, M, N);
        unDPCM(unrleData.Cr, M, N);
        ...
        // 執行 unquantization
        unquantizeDCT(unrleData.Y, QY, addheight, addwidth);
        unquantizeDCT(unrleData.Cb, QCbCr, addheight, addwidth);
        unquantizeDCT(unrleData.Cr, QCbCr, addheight, addwidth);

        // Generate basis vectors for IDCT
        double basis_vector[DCT_H][DCT_W][DCT_H][DCT_W];
        generateBasisVectors(basis_vector);

        // Allocate memory for the final image
        double **imageY = allocateImageMemory(addheight, addwidth);
        double **imageCb = allocateImageMemory(addheight, addwidth);
        double **imageCr = allocateImageMemory(addheight, addwidth);

        // 執行 inverse DCT
        inverseDCT(unrleData.Y, imageY, basis_vector, addheight, addwidth);
        inverseDCT(unrleData.Cb, imageCb, basis_vector, addheight, addwidth);
        inverseDCT(unrleData.Cr, imageCr, basis_vector, addheight, addwidth);

        // Allocate memory for RGB images
        unsigned char **imageR = (unsigned char **)malloc(addheight * sizeof(unsigned char *));
        unsigned char **imageG = (unsigned char **)malloc(addheight * sizeof(unsigned char *));
        unsigned char **imageB = (unsigned char **)malloc(addheight * sizeof(unsigned char *));
        for (int i = 0; i < addheight; i++)
        {
            imageR[i] = (unsigned char *)malloc(addwidth * sizeof(unsigned char));
            imageG[i] = (unsigned char *)malloc(addwidth * sizeof(unsigned char));
            imageB[i] = (unsigned char *)malloc(addwidth * sizeof(unsigned char));
        }

        // Convert YCbCr to RGB
        YCbCrToRGB(imageY, imageCb, imageCr, imageR, imageG, imageB, addheight, addwidth);
        ...
        // Write pixel data
        writePixelData(fp, imageR, imageG, imageB, width, height);

        // Close file
        fclose(fp);

        // 釋放記憶體
        freeRLEImage(rleImage, height, width);
        freeUnRLEData(unrleData, M, N);
        // Free allocated memory
        freeImageMemory(imageY, height);
        freeImageMemory(imageCb, height);
        freeImageMemory(imageCr, height);
        freeImageMemory((double **)imageR, height); // Casting to double** for compatibility
        freeImageMemory((double **)imageG, height);
        freeImageMemory((double **)imageB, height);
        printf("RLE decode completed");
    }
    else
    {
